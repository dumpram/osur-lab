...
/*!
 * Start new process
 * \param pid PID of created process
 * \param path Program name ("file name")
 * \param file_actions not in use
 * \param attrp not in use
 * \param argv Command line arguments for starting thread (if not NULL)
 * \param envp not in use
 * \return 0 if process is created, -1 otherwise
 */
int sys__posix_spawn ( void *p )
{
	pid_t *pid;
	char *path;
	/* void *file_actions;	not used */
	/* void *attrp;		not used */
	char **argv;		/* argument list */
	/* char **envp;		not used */

	kthread_t *kthread;
	void *proc = kthread_get_process (NULL);
	char *arg, *karg, **args, **kargs = NULL;
	int argnum, argsize;

	pid =		*( (pid_t **) p );		p += sizeof (pid_t *);
	path =		*( (char **) p );		p += sizeof (char *);
	/* file_actions =*( (void **) p ); */		p += sizeof (void *);
	/* attrp =	*( (void **) p ); */		p += sizeof (void *);
	argv =		*( (char ***) p );		p += sizeof (char **);
	/* envp =	*( (char ***) p ); */

	ASSERT_ERRNO_AND_EXIT ( path, ESRCH );
	path = U2K_GET_ADR ( path, proc );

	if ( argv ) /* copy parameters from one process space to another */
	{
		/* copy parameters to new process address space */
		/* first copy them to kernel */
		argnum = 0;
		argsize = 0;
		args = U2K_GET_ADR ( argv, proc );
		while ( args[argnum] )
		{
			arg = U2K_GET_ADR ( args[argnum++], proc );
	...
	kthread = kthread_start_process ( path, kargs, 0 );

	if ( !kthread )
		EXIT ( ENOMEM );

	if ( pid ) /* save thread descriptor */
	{
		pid = U2K_GET_ADR ( pid, proc );
		pid->ptr = kthread;
...
int sys__pthread_getspecific ( void *p ) {
    ...
    kproc = kthread_get_process (NULL);
    key = U2K_GET_ADR ( key, kproc );
    value = U2K_GET_ADR ( value, kproc ); // translate process logic address
    ...
    if ( thread_key == NULL ) {
        ...
        *value = thread_key->value;
        key->id = thread_key->id;
        key->destructor = thread_key->destructor;
        key->value = (void *)thread_key->value;
    }
    return 0;
}

int sys__pthread_key_delete ( void *p ) {
    pthread_key_t key;
    kprocess_t *kproc; // current process
    kthread_t *curr_thread; // current thread
    kprocess_key_t *proc_key;
    ...
    if ( thread_key->id != NULL ) {
        list_remove ( thread_keys, 0, &thread_key->list );
        proc_key->ref_cnt--;
        if ( proc_key->ref_cnt == 0 ) {
            list_remove ( &kproc->keys, 0, &proc_key->list );
        }
    }
