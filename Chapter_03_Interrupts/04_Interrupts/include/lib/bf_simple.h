...
#ifndef _BF_SIMPLE_C_

typedef void bfs_mpool_t;
...
typedef struct _bfs_mpool_t_
{
	bfs_hdr_t *first;
}
bfs_mpool_t;

#define HEADER_SIZE ( sizeof (bfs_hdr_t) + sizeof (bfs_tail_t) )
...
#define GET_TAIL(HDR)	(GET_AFTER(HDR) - sizeof (bfs_tail_t))
#define GET_HDR(TAIL)	(((void *)(TAIL)) - GET_SIZE(TAIL) + sizeof(bfs_tail_t))

#define CLONE_SIZE_TO_TAIL(HDR)	\
	do { ( (bfs_tail_t *) GET_TAIL(HDR) )->size = (HDR)->size; } while(0)
